    ...
    if args.task == "nc":
        ...
        if args.task == "lp":
            (
                ...
                test_edges,
                test_edges_false,
            ) = mask_edges(adj, args.val_prop, args.test_prop, args.split_seed)
            data["adj_train"] = adj_train
            data["train_edges"], data["train_edges_false"] = (
                train_edges,
                train_edges_false,
            )
...
def split_data(labels, val_prop, test_prop, seed):
    ...
    idx_val_pos, idx_test_pos, idx_train_pos = (
        pos_idx[:nb_val],
        pos_idx[nb_val : nb_val + nb_test],
        pos_idx[nb_val + nb_test :],
    )
    idx_val_neg, idx_test_neg, idx_train_neg = (
        neg_idx[:nb_val],
        neg_idx[nb_val : nb_val + nb_test],
        neg_idx[nb_val + nb_test :],
    )
    return (
        idx_val_pos + idx_val_neg,
        idx_test_pos + idx_test_neg,
        idx_train_pos + idx_train_neg,
    )
...
def load_citation_data(dataset_str, use_feats, data_path, split_seed=None):
    ...
    for i in range(len(names)):
        with open(
            os.path.join(data_path, "ind.{}.{}".format(dataset_str, names[i])), "rb"
        ) as f:
...
def load_synthetic_data(dataset_str, use_feats, data_path):
    ...
    for i, j in edges:
        adj[i, j] = 1.0  # comment this line for directed adjacency matrix
        adj[j, i] = 1.0
    if use_feats:
        features = sp.load_npz(
            os.path.join(data_path, "{}.feats.npz".format(dataset_str))
        )
    else:
        features = sp.eye(adj.shape[0])
