...
class Lorentz_attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., manifold='Lorentz'):
        super().__init__()
        ...
        self.qkv = LorentzLinear(in_features=dim, out_features=dim * 3, bias=qkv_bias,dropout=0.0, manifold=manifold)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = LorentzLinear(in_features=dim, out_features=dim,manifold=manifold,dropout=proj_drop)
...
class Lorentz_ViT(nn.Module):
    def __init__(self, image_resolution : list, num_classes : int = 10, num_blocks : int = 12,in_dim : int = 3, channels : int = 768, act : str = 'gelu',manifold : str = 'Lorentz', dropout : float=0.1,scale : int = 10,):
        super().__init__()
        self.in_dim = in_dim
        ...
        for i in range(self.num_blocks):
            self.backbone += [
                Lorentz_VIT_Block(dim = self.channels, manifold=self.manifold,patches=self.patches,scale=scale)
            ]
            idx += 1

        self.backbone = Seq(*self.backbone)
        self.lorentz_centroid = Lorentz_centroid(
            dim=self.channels, n_classes=self.num_classes, bias=True, manifold=self.manifold
        )
        self.rearrange = Rearrange('b p c -> b c p')
