...
from .utils import acosh, clamp, sqrt
...
def _inner(u, v, keepdim: bool = False, dim: int = -1):
    ...
    uv = u * v
    if keepdim is False:
        return -uv.narrow(dim, 0, 1).squeeze(dim) + uv.narrow(dim, 1, d).sum(
...
def _project(x, k: torch.Tensor, dim: int = -1):
    ...
    left_ = torch.sqrt(k + (right_ * right_).sum(dim=dim, keepdim=True))
...
def _logmap0back(x, k, dim: int = -1):
    ...
    nomin = torch.cat((nomin_.narrow(dim, 0, 1) + 1, nomin_.narrow(dim, 1, dn)), dim)
    denom = _norm(nomin, keepdim=True)
    return dist_ * nomin / denom
...
def _geodesic_unit(t, x, u, k):
    return torch.cosh(t) * x + torch.sinh(t) * u
...
def poincare_to_lorentz(x, k, dim=-1, eps=1e-6):
    ...
    res = torch.cat((1 + x_norm_square, 2 * x), dim=dim) / (1.0 - x_norm_square + eps)
    return res
